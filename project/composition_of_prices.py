...
def get_table_of_composition(angebote_id):
    ...
    first_level = db.session.query(
        ...
        angebote5.preis.label("preis5"), (angebote5.preis*(produktionsmittel4.prozent_gebraucht/100)).label("kosten5"),
        produktionsmittel5.prozent_gebraucht.label("proz_gebr6"), produktionsmittel5.kauf.label("kauf6"))\
    ...
    fifth_level = fourth_level.outerjoin(kaeufe5, produktionsmittel4.kauf==kaeufe5.id).outerjoin(angebote5, kaeufe5.angebot==angebote5.id).\
...
def get_positions_in_table(base_query):
    ...
    stores the info, in which row and column of the database table the angebote are positioned
    """
...
def create_dots(cols_dict, table_of_composition):
    ...
    the database table. If angebot x is in the same row and next column of angebot y,
    x is child of y and will be connected with an edge.
    """
