...
def upgrade() -> None:
    ...
    SELECT 
        ...
        t.amount_sent,
        CASE
            WHEN t.sending_account = c.p_account THEN '{TransferType.productive_consumption_p.value}'::transfertype
            WHEN t.sending_account = c.r_account THEN '{TransferType.productive_consumption_r.value}'::transfertype
    ...
    WHERE tr.temp_original_transaction_id_for_migration_pc = prod_c.transaction_id
      AND tr.type IN ('{TransferType.productive_consumption_p.value}'::transfertype, '{TransferType.productive_consumption_r.value}'::transfertype);
    ''')
    ...
    SELECT
        ...
        CASE
            WHEN cd.compensation_value > 0 THEN '{TransferType.compensation_for_coop.value}'::transfertype
            ELSE '{TransferType.compensation_for_company.value}'::transfertype
        END,
    ...
    FROM transfer tr
    WHERE tr.temp_original_transaction_id_for_migration_pc = prod_c.transaction_id
      AND tr.type IN ('{TransferType.compensation_for_coop.value}'::transfertype, '{TransferType.compensation_for_company.value}'::transfertype);
    ''')
    ...
    WHERE EXISTS (
        ...
        WHERE main_tr.date = t.date 
          ...
          AND main_tr.type IN ('{TransferType.productive_consumption_p.value}'::transfertype, '{TransferType.productive_consumption_r.value}'::transfertype)
...
def downgrade() -> None:
    ...
    SELECT 
        ...
        CASE
            WHEN t_comp.id IS NOT NULL AND t_comp.type = '{TransferType.compensation_for_coop.value}'::transfertype THEN t_pc.value - t_comp.value
            WHEN t_comp.id IS NOT NULL AND t_comp.type = '{TransferType.compensation_for_company.value}'::transfertype THEN t_pc.value + t_comp.value
    ...
    WHERE t_pc.type IN ('{TransferType.productive_consumption_p.value}'::transfertype, '{TransferType.productive_consumption_r.value}'::transfertype);
    ''')
    ...
    WHERE prod_c.transfer_of_productive_consumption = t_pc.id
      ...
      AND t_pc.type IN ('{TransferType.productive_consumption_p.value}'::transfertype, '{TransferType.productive_consumption_r.value}'::transfertype)
    ...
    ) AND t.type IN ('{TransferType.compensation_for_coop.value}'::transfertype, '{TransferType.compensation_for_company.value}'::transfertype);
    ''')
    op.execute(f'''
    DELETE FROM transfer t
    WHERE t.id IN (
    ...
    ) AND t.type IN ('{TransferType.productive_consumption_p.value}'::transfertype, '{TransferType.productive_consumption_r.value}'::transfertype);
    ''')
