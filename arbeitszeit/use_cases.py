...
def purchase_product(
    ...
    lookup_price: Callable[[ProductOffer], Decimal],
    ...
    price = lookup_price(product_offer)
    ...
    if product_offer.amount_available < amount:
        raise Exception("Amount ordered exceeds available products!")
    elif product_offer.amount_available == amount:
        product_offer.deactivate()

    provider = lookup_product_provider(product_offer)
    provider.increase_credit(price * amount, "balance_prd")
