...
class ControllerTests(BaseTestCase):
    ...
    def test_plan_id_string_in_uuid_format_gets_converted_to_interactor_request(
        self,
    ) -> None:
    ...
    def test_plan_id_string_in_uuid_format_gets_converted_to_interactor_request_with_correct_plan_id(
        self,
    ) -> None:
        expected_plan_id = uuid4()
        plan_id = str(expected_plan_id)
        interactor_request = self.controller.create_request(plan_id)
        self.assertEqual(interactor_request.plan_id, expected_plan_id)
