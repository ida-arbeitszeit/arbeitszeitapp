...
class AcceptCoordinationTransferControllerTests(BaseTestCase):
    ...
    def test_transfer_request_id_gets_passed_to_use_case_request(self) -> None:
        expected = uuid4()
        uc_request = self.controller.create_use_case_request(transfer_request=expected)
        self.assertEqual(expected, uc_request.transfer_request_id)

    def test_current_logged_in_company_gets_passed_to_use_case_request_as_accepting_company(
        self,
    ) -> None:
        uc_request = self.controller.create_use_case_request(transfer_request=uuid4())
