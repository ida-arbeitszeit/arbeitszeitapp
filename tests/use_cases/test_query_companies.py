...
class TestQueryCompanies(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.use_case = self.injector.get(QueryCompaniesUseCase)
    ...
    def test_that_no_company_is_returned_when_searching_an_empty_repository(self):
        response = self.use_case.execute(make_request(None, CompanyFilter.by_name))
        assert not response.results
        response = self.use_case.execute(make_request(None, CompanyFilter.by_email))
        assert not response.results
    ...
    def test_that_company_is_returned_when_searching_with_empty_query_string(self):
        ...
        response = self.use_case.execute(make_request("", CompanyFilter.by_name))
        assert self.company_in_results(expected_company, response)
    ...
    def test_query_substring_of_name_returns_correct_result(self):
        ...
        response = self.use_case.execute(make_request("Company", CompanyFilter.by_name))
        assert self.company_in_results(expected_company, response)
    ...
    def test_that_companies_where_name_not_match_are_not_returned(self):
        ...
        response = self.use_case.execute(make_request("Factory", CompanyFilter.by_name))
        assert not response.results
...
class TestPagination(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.query_companies = self.injector.get(QueryCompaniesUseCase)
