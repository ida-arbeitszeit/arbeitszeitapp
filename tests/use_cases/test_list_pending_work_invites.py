...
from arbeitszeit.use_cases import invite_worker_to_company, list_pending_work_invites
...
class ListPendingWorkInvitesTests(BaseTestCase):
    def setUp(self) -> None:
        ...
        self.use_case = self.injector.get(
            list_pending_work_invites.ListPendingWorkInvitesUseCase
        )
    ...
    def invite_worker(self, company: UUID, worker: UUID) -> None:
        ...
        response = use_case.invite_worker(request=request)
    ...
    def list_pending_invites(self, company: UUID) -> list_pending_work_invites.Response:
        ...
        return self.use_case.list_pending_work_invites(request=request)
