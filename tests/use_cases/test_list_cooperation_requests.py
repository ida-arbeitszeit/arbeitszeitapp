...
def plan_in_list(plan: Plan, response: ListCooperationRequestsResponse) -> bool:
...
def test_emtpy_list_is_returned_when_cooperation_is_not_found(
    list_requests: ListCooperationRequests,
):
    response = list_requests(ListCooperationRequestsRequest(uuid4()))
...
def test_empty_list_is_returned_when_there_are_no_requests_for_coordinator(
    ...
    response = list_requests(ListCooperationRequestsRequest(coordinator.id))
    assert len(response.cooperation_requests) == 0
