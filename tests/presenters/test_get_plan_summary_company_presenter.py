...
class GetPlanSummaryCompanySuccessPresenterTests(TestCase):
    ...
    def test_url_for_ending_cooperation_is_displayed_correctly_when_plan_is_cooperating(
        ...
        assert TESTING_RESPONSE_MODEL.cooperation
        view_model = self.presenter.present(TESTING_RESPONSE_MODEL)
        self.assertEqual(
            view_model.action.is_cooperating, TESTING_RESPONSE_MODEL.is_cooperating
        )
        self.assertEqual(
            view_model.action.end_coop_url,
            self.end_coop_url_index.get_end_coop_url(
                TESTING_RESPONSE_MODEL.plan_id, TESTING_RESPONSE_MODEL.cooperation
            ),
        )
    ...
    def test_url_for_ending_cooperation_is_displayed_correctly_when_plan_is_not_cooperating(
        ...
        response = replace(
            TESTING_RESPONSE_MODEL, is_cooperating=False, cooperation=None
        )
        view_model = self.presenter.present(response)
        self.assertEqual(view_model.action.is_cooperating, response.is_cooperating)
